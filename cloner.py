    ...
    async def roles_delete(guild_to: discord.Guild):
        for role in guild_to.roles:
            try:
                ...
                print_error(
                    f"Não foi possivel deletar o cargo {Fore.YELLOW}{role.name}{Fore.RED} Erro não identificado"
                )
    ...
    async def roles_create(guild_to: discord.Guild, guild_from: discord.Guild):
        ...
        for role in roles:
            try:
                ...
                print_error(
                    f"Não foi possivel criar a cargo {Fore.YELLOW}{role.name}{Fore.RED} Erro não identificado"
                )
                await asyncio.sleep(random.randint(9, 12))
            ...
            try:
                for category in guild_to.categories:
                    try:
                        ...
                        print_warning(
                            f"Canal {channel_text.name} não tem nenhuma categoria!"
                        )
                        category = None
                        break
    ...
    async def emojis_create(guild_to: discord.Guild,
        ...
        for emoji in guild_from.emojis:
            try:
                ...
                print_error(
                    f"Não foi possivel criar o emoji {Fore.YELLOW}{emoji.name}{Fore.RED} Erro não identificado"
                )
                await asyncio.sleep(random.randint(9, 12))
    ...
    async def guild_edit(guild_to: discord.Guild, guild_from: discord.Guild):
        try:
            try:
                ...
                print_error(
                    f"Não é possível ler a imagem do ícone de {guild_from.name}"
                )
                icon_image = None
